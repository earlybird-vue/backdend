...
class Documents
{
    ...
    public function run(){
        ...
        if(is_file($this->template['view_path'].$this->config['template'].'.html')){
            echo $this->view->fetch($this->config['template']);
        } else {
    ...
    private function Item($class,$method)
    {
        ...
        return [
            ...
            'return_demo'=>isset($item['return_demo'])&&!empty($item['return_demo'])?$item['return_demo']:'未配置返回示例',
            'postBody'=>isset($item['postBody'])&&!empty($item['postBody'])?$item['postBody']:'',
    ...
    private function getData($res){
        ...
        foreach($res as $key=>$val){
            ...
            if($key=='@desc'){
                //$tt = array("0"=>"请求方式","3"=>array("code"=>200,"data"=>array("list"=>array(array("id"=>1001,"name"=>"lly"),array("id"=>1001,"name"=>"lly"))),"err"=>""));
                //echo json_encode($tt,JSON_UNESCAPED_UNICODE);
                //echo $val;die();
                $tmp_arr = json_decode($val,true);
                $json_str = '';
                foreach($tmp_arr as $tmpkey=>$v)
                {
                    if(is_array($v)){
                        //$tmpstr = json_encode($v);
                        //str_replace('{','{\n',$tmpstr);
                        //echo $tmpstr;
                        $json_str = '<pre>'.json_encode($v,JSON_PRETTY_PRINT).'</pre>';
                        unset($tmp_arr[$tmpkey]);
                    }
                }
                $description=implode("<br>",(array)$tmp_arr).$json_str;
            }
            if($key == '@returnDemo'){
                $tmp_arr = json_decode($val,true);
                $returnDemo = '<pre>'.json_encode($tmp_arr[0],JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT).'</pre>';
            }
            if($key == '@postBody'){
                $tmp_arr = json_decode($val,true);
                $postBody = '<pre>'.json_encode($tmp_arr[0],JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT).'</pre>';
            }
            if($key=='@param'){
                $param=$val;
            }
        ...
        return array('title'=>$title,'desc'=>$description,'params'=>$params,'returns'=>$returns,'return_demo'=>$returnDemo,'postBody'=>$postBody);
    }
}